...
const nextConfig = {
  ...
  experimental: {
    ...
    outputFileTracingRoot: "/Users/shanil/Projects/Personal/lens/lens",
...
const formatKey = (key) => {
  return getColoredText(key, 34);
};
const formatValue = (value) => {
  return getColoredText(value, 32);
};
...
db.createTables()
  ...
  .then(() => {
    ...
    const formattedHostname = "localhost";
    const appUrl = `http://${formattedHostname}:${port}`;
    console.log(`✓ Model Lens is available on ${formatValue(appUrl)} 
○ Please add the following to your .env file in your langchain project
  ${formatKey("LANGCHAIN_TRACING_V2")}=${formatValue("true")}
  ${formatKey("LANGCHAIN_ENDPOINT")}="${formatValue(`${appUrl}/api`)}"
or
  ${formatValue("EXPORT")} ${formatKey("LANGCHAIN_TRACING_V2")}=${formatValue(
      "true"
    )}
  ${formatValue("EXPORT")} ${formatKey("LANGCHAIN_ENDPOINT")}="${formatValue(
      `${appUrl}/api`
    )}"
before running your langchain project
    `);
  })
  .catch((err) => {
    console.error(err);
